...
params0 <- expand.grid(n = n, lambda = lambda, alpha = alpha)
...
simFun <- function(sub){
  ...
  for (i in 1:nrow(sub)){
    ...
    n <- sub$n[i]
    x <- rgamma(n = n, shape = sub$alpha[i], rate = sub$lambda[i])
    y <- rgamma(n = n, shape = sub$alpha[i], rate = sub$lambda[i])
    ...
    try({
      sub$gammaDiff[i] <- 1-pgammaDif(t0, n, n, alpha = sub$alpha[i], lambda = sub$lambda[i]) +
                        pgammaDif(-t0, n, n, alpha = sub$alpha[i], lambda = sub$lambda[i])
    })
    try({
      sub$gammaDiffSaddle[i] <- 
          pgammaDifSaddle(t0, n, n, alpha = sub$alpha[i], lambda = sub$lambda[i], lower.tail = FALSE) +
          pgammaDifSaddle(-t0, n, n, alpha = sub$alpha[i], lambda = sub$lambda[i], lower.tail = TRUE)
    })
    ...
    t0 <- mean(x) - mean(y)
    tStud0 <- t0 / sqrt(var(x) / n + var(y) / n)
    for (b in 1:Bmc) {
      star <- sample(1:N, replace = FALSE)
      xStar <- z[star[1:n]]
      yStar <- z[star[(n+1):N]]
      T[b] <- mean(xStar) - mean(yStar)
      Tstud[b] <- (mean(xStar) - mean(yStar)) / sqrt(var(xStar) / n + var(yStar) / n)
    }
...
save(symResults, file ="symResultsGammaDiff_null.RData")
