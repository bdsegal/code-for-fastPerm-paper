...
simFun <- function(sub){
  ...
  for (i in 1:nrow(sub)){
    ...
    try({
      sub$gammaDiff[i] <- 1-pgammaDif(t0, n, n, alpha = sub$alpha[i], lambda = mle[2]) +
                            pgammaDif(-t0, n, n, alpha = sub$alpha[i], lambda = mle[2])
    })
    try({
      sub$gammaDiffSaddle[i] <- 
          pgammaDifSaddle(t0, n, n, alpha = sub$alpha[i], lambda = mle[2], lower.tail = FALSE) +
          pgammaDifSaddle(-t0, n, n, alpha = sub$alpha[i], lambda = mle[2], lower.tail = TRUE)
    })
...
save(symResults, file ="symResultsGammaDiff_smallN.RData")
